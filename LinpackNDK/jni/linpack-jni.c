...
jint
Java_com_bench_linpack_LinpackNDK_getLineCountJNI(
				JNIEnv* env, 
				jobject thiz) {
	return outputLine;
}
...
jstring
Java_com_bench_linpack_LinpackNDK_getResultJNI(
				JNIEnv* env, 
				jobject thiz,
				jint i) {
...
jint
Java_com_bench_linpack_LinpackNDK_testJNI(
				JNIEnv* env, 
				jobject thiz,
				jint i) {
    char    buf[80];
...
//        printf("Memory required:  %ldK.\n",(memreq+512L)>>10);
        ...
        if (malloc_arg!=memreq || (mempool=malloc(malloc_arg))==NULL) {
            return memreq;
__android_log_print(ANDROID_LOG_INFO, "************************", "mempool = %d: \n", (int)mempool);
/*
...
static REAL linpack(long nreps,int arsize)
    ...
    if (toverhead<0.)
        toverhead=0.;
    sprintf(resultString[outputLine], 
