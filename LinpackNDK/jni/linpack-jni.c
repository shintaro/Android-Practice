...
static double time[9][9];
static char tmp[19][256];
 ...
 */
jstring
Java_com_bench_linpack_LinpackNDK_getResultJNI(JNIEnv* env, jobject thiz,
                                        jint i) {
	return (*env)->NewStringUTF(env, tmp[(int)i]);
}

jint
Java_com_bench_linpack_LinpackNDK_testJNI(JNIEnv* env, jobject thiz) {

	static REAL aa[SIZE][SIZE],a[SIZE][SIZE+1],b[SIZE],x[SIZE];
	//REAL cray,ops,total,norma,normx;
	REAL norma,normx;
	double cray,ops,total;
...
//	fprintf(result[0],"%s%sPrecision Linpack\n\n",ROLLING,PREC);
//	fprintf(stderr,ROLLING);fprintf(stderr,PREC);fprintf(stderr,"Precision Linpack\n\n");
//	fprintf(stderr,ROLLING);fprintf(stderr,PREC);fprintf(stderr,"Precision Linpack\n\n");
	sprintf(tmp[0],"Rolled Single Precision Linpack\n");
	sprintf(tmp[1],"\n");
	sprintf(tmp[2],"Rolled Single Precision Linpack\n");
	sprintf(tmp[3],"\n");
...
/*     compute a residual to verify results.  */ 
	...
	sprintf(tmp[4],"     norm. resid      resid           machep         x[0]-1        x[n-1]-1\n");
	sprintf(tmp[5],"  %8.1f      %16.8e%16.8e%16.8e%16.8e\n",
	       (double)residn, (double)resid, (double)eps, 
               (double)x[0]-1, (double)x[n-1]-1);

   	sprintf(tmp[6],"    times are reported for matrices of order %5d\n",n);
	sprintf(tmp[7],"      dgefa      dgesl      total       kflops     unit      ratio\n");
	...
	time[2][0] = total;
  	...
  	sprintf(tmp[8]," times for array with leading dimension of%5d\n",lda);
	print_time(0, 9);
	...
	sprintf(tmp[13]," times for array with leading dimension of%4d\n",ldaa);
	print_time(4, 14);
	print_time(5, 15);
	print_time(6, 16);
	print_time(7, 17);
//	sprintf(stderr,ROLLING);fprintf(stderr,PREC);
	sprintf(tmp[18]," Precision %5d Kflops ; %d Reps \n",kflops,NTIMES);
...
int row; int buf;
{
sprintf(tmp[buf],"%11.2f%11.2f%11.2f%11.0f%11.2f%11.2f\n",   (double)time[0][row],
