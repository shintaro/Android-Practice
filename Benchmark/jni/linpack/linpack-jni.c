...
#ifdef ANDROID
static REAL linpack  (long nreps,int arsize,JNIEnv *env,jobject thiz);
#else
...
jint
Java_com_agatsuma_android_benchmark_Linpack_runLinpack(
				JNIEnv* env, 
				jobject thiz,
				jint i) 
#else
...
/*
        ...
        sprintf(resultString[1], 
            "Machine precision:  %d digits.\n",BASE10DIG);
        sprintf(resultString[2], "Array size %d X %d.\n",arsize,arsize);
        sprintf(resultString[3], "Average rolled and unrolled performance:\n\n");
        sprintf(resultString[4], 
            "    Reps Time(s) DGEFA   DGESL  OVERHEAD    KFLOPS\n");
        sprintf(resultString[5], 
            "----------------------------------------------------\n");
        outputLine = 6;
*/

#ifndef ANDROID
        printf("(J\(Bn(J\(BnLINPACK benchmark, %s precision.(J\(Bn",PREC);
        printf("Machine precision:  %d digits.(J\(Bn",BASE10DIG);
        printf("Array size %d X %d.(J\(Bn",arsize,arsize);
        printf("Average rolled and unrolled performance:(J\(Bn(J\(Bn");
        printf("    Reps Time(s) DGEFA   DGESL  OVERHEAD    KFLOPS(J\(Bn");
        printf("----------------------------------------------------(J\(Bn");
#endif
...
#ifdef ANDROID
	while (linpack(nreps,arsize,env,thiz)<10.)
#else
...
static REAL linpack(long nreps,int arsize)
    ...
    jstring jstr = (*env)->NewStringUTF(env, resultString);
    jclass clazz = (*env)->FindClass(env, "com/agatsuma/android/benchmark/Linpack");
    jmethodID callBackMethod = (*env)->GetMethodID(env, clazz,
                    "setResultStringFromNative",
                    "(Ljava/lang/String;)V");
    (*env)->CallObjectMethod(env, thiz, callBackMethod, jstr);
#endif
