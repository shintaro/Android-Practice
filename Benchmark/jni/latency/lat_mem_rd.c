...
/*
JNIEXPORT void JNICALL Java_Caller_callCallBackMethod
(JNIEnv* env, jobject thiz, jobject methods, jstring str) {
	jclass clazz = (*env)->GetObjectClass(env, methods);
	jmethodID mid = (*env)->GetMethodID(env, clazz, "callBackMethod",
				 "(Ljava/lang/String;)V");
...
jdouble
Java_com_agatsuma_android_benchmark_Latency_loads(
                                JNIEnv* env, 
                                jobject thiz,
                                jlong len,
                                jint range,
	...
	state.width = 1;
	state.len = (size_t)range;
	state.maxlen = (size_t)len;
	state.line = (size_t)stride;
...
//	Java_Caller_callCallBackMethod(*env, thiz, methods, "JNI\n");
/*
	jstring jstr = (*env)->NewStringUTF(env, "This is JNI\n");
	jclass clazz = (*env)->FindClass(env, "com/bench/lmbench/LMBench");
	jmethodID callBackMethod = (*env)->GetMethodID(env, clazz, "callBackMethod", "(Ljava/lang/String;)V");
